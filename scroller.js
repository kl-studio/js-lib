...
const scroller = {
    ...
    intersectionCallback: (entries) => {
        ...
        entries.forEach(entry => {
            ...
            if(entry.isIntersecting) {
                ...
                window.addEventListener("scroll", scroller.render.bind(null, entry.target));
            ...
            if(parallax) {
                ...
                parallax.forEach(elem => {
                    ...
                    if(elem.classList.contains('para-one')) {
                        elem.style.transform = `translateY(${scrollCalc * 0.4}px) translateX(${scrollCalc * 1}px)`;
                    }

                    if(elem.classList.contains('para-two')) {
                        elem.style.transform = `translateY(${scrollCalc * 0.45}px) translateX(${scrollCalc * -1}px)`;
                    }

                    if(elem.classList.contains('para-three')) {
                        elem.style.transform = `translateY(${scrollCalc * 0.5}px)`;
                    }
    ...
    init: (state) => {
        ...
        lib.qsa('.can-intersect').forEach((elem) => {
            ...
            let observer = new IntersectionObserver(scroller.intersectionCallback, options)
